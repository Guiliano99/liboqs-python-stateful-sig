...
from typing import TYPE_CHECKING, Any, ClassVar, Final, TypeVar, Union, cast, Optional, Iterable
...
def _check_alg(alg_name: str) -> None:
    ...
    if alg_name not in _supported_stateful_sigs:
        ...
        if _filtered_names is None:
            raise MechanismNotSupportedError(alg_name)
        raise MechanismNotSupportedError(alg_name + f", supported names: {_filtered_names}")
    if alg_name not in _enabled_stateful_sigs:
        sup = _filter_stfl_names(alg_name, _enabled_stateful_sigs)
        raise MechanismNotEnabledError(alg_name + f", enabled names: {sup}")
...
class StatefulSignature(ct.Structure):
    ...
    _fields_: ClassVar[Sequence[tuple[str, Any]]] = [
        ...
        ("length_secret_key", ct.c_size_t),
        ("length_signature", ct.c_size_t),
        ("keypair", ct.c_void_p),  # opaque fn-ptrs
        ("sign", ct.c_void_p),
        ("sign_with_ctx_str", ct.c_void_p),
        ("verify", ct.c_void_p),
        ("verify_with_ctx_str", ct.c_void_p),
    ]
    ...
    def __enter__(self) -> "StatefulSignature":
